...
class StackPyramid(Task):
    ...
    def _create_scene(self) -> None:
        ...
        self.sim.create_box(
            ...
            mass=1.0,
            position=np.array([0.5, 0.0, self.object_size / 2]),
            rgba_color=np.array([1.0, 1.0, 0.1, 1.0]),
        )
        self.sim.create_box(
            body_name="target3",
            half_extents=np.ones(3) * self.object_size / 2,
            ...
            ghost=True,
            position=np.array([0.5, 0.0, 0.05]),
            rgba_color=np.array([1.0, 1.0, 0.0, 0.3]),
        )
    ...
    def _sample_goal(self) -> np.ndarray:
        ...
        goal3 = np.array([0.0, 0.0, 3 * self.object_size / 2])  # z offset for the cube center
        noise = self.np_random.uniform(self.goal_range_low, self.goal_range_high)
        goal1 += noise
        goal2 += noise + np.array([0.0, 3 * self.object_size / 2 , 0.0])
        goal3 += noise + np.array([0.0, self.object_size / 2 , 0.0])
