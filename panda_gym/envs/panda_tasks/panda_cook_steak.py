...
class PandaCookSteak(RobotTaskEnv):
    ...
    def __init__(self, render: bool = False, debug:bool = False, reward_type: str = "sparse", control_type: str = "ee") -> None:
        ...
        robot1 = Panda(sim, block_gripper=False, base_position=np.array([-0.65, -0.1, 0.4]), base_orientation=np.array([0, 0, -0.5]), control_type=control_type, body_name="_left")
        robot2 = Panda(sim, block_gripper=False, base_position=np.array([0.35, -0.1, 0.4]), base_orientation=np.array([0, 0, 2.8]), control_type=control_type, body_name="_right", base_gripper_orientation=np.array([np.pi,0.,np.pi]))
